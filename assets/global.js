...
class QuantityInput extends HTMLElement {
  constructor() {
    ...
    this.input.setAttribute('data-count', '1');
  ...
  onButtonClick(event) {
    ...
    this.input.setAttribute('data-count', this.input.value);
  }
...
class MenuDrawer extends HTMLElement {
  constructor() {
    ...
    this.closeMenu = this.querySelector('.menu-drawer__title span');

    if (this.closeMenu) {
      this.closeMenu.addEventListener('click', this.onCloseMenuClick.bind(this));      
    }
  ...
  onCloseMenuClick(event) {
    this.closeMenuDrawer(event, this.closeMenu);
...
class VariantSelects extends HTMLElement {
  constructor() {
    ...
    inputWrappers.forEach((option, index) => {
      if (index === 0) return;
      const options = [...option.querySelectorAll('input[type="radio"], option')]
      const availableOptions = Array.from(document.querySelectorAll('.sellect__option--2 .option__value')).map(input => input.value);
      this.setInputAvailability(options, availableOptions);
    });
  ...
  setInputAvailability(listOfOptions, listOfAvailableOptions) {
    listOfOptions.forEach(input => {
      if (listOfAvailableOptions.includes(input.getAttribute('value'))) {
        ...
        input.innerText = 'Choose an option';
